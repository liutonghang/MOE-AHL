...
from models.moe_ahl import build_model
from core.metric import MetricsTop
import yaml

parser = argparse.ArgumentParser()
# parser.add_argument('--config_file', type=str, required=True)  # 改为必须参数
parser.add_argument('--config_file', type=str, default='configs/mosi.yaml')

parser.add_argument('--seed', type=int, default=-1)
parser.add_argument('--gpu_id', type=int, default=0)
parser.add_argument('--output_file', type=str, default='')  # 新增：结果输出文件
...
def main():
    ...
    if opt.output_file:
        with open(opt.output_file, 'a+') as f:
            f.write(f"Config: {opt.config_file}\n")
            f.write(f"Seed: {seed}\n")
            f.write(f"Best Validation Results: {best_validation_results['best_results_all_epochs']}\n")
            f.write(f"Best Test Results: {best_test_results['best_results_all_epochs']}\n")
            f.write("----------------------------------------\n")
...
def train(model, data_loader, optimizer, loss_fn, metrics_fn):
    ...
    for cur_iter, data in enumerate(data_loader):
        ...
        load_balance_loss = model.get_load_balancing_loss()
        loss = loss_fn(output, label) + 0.1 * load_balance_loss
